...
P_AUTH_EXEMPT_EXACT = {
    ...
    "/api/version",
    # Local Google OAuth token-endpoint proxy: hit by the google-workspace-mcp subprocess we spawn. It doesn't (and can't easily) carry the install bearer in google-auth's refresh post. Localhost binding is the gate, and the route does nothing the OAuth broker's /api/oauth/google/refresh doesn't already do for any internet caller, so no new attack surface.
