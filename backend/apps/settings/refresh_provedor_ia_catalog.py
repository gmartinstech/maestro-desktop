"""Fetch the provedor-ia catalog, then re-derive the provider entry from it.

The seam both callers share: the settings lifespan (so a launch picks up models
added server-side) and POST /provedor-ia/token (so a fresh sign-in seeds the live
...
@typechecked
async def refresh_provedor_ia_catalog(settings: AppSettings) -> bool:
    ...
    logger.debug("provedor-ia catalog refreshed: %d models", len(models))
    return True
