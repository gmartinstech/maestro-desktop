...
async def configure_provider_env(
    ...
    if is_pinned_api_route and api_route_provider == "anthropic" and getattr(global_settings, "anthropic_api_key", None):
        ...
        # Dead provedor-ia session: fail as auth HERE, before 9Router is even consulted, or the missing node surfaces as "that model may not exist" (see guard_maestro_session for the full chain).
        from backend.apps.agents.manager.guard_maestro_session import guard_maestro_session as p_guard_pi
