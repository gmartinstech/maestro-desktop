...
def save_session(session_id: str, doc_data: dict):
    sessions_dir = p_sessions_dir()
...
def load_session_data(session_id: str) -> dict | None:
    return read_json_or_none(os.path.join(p_sessions_dir(), f"{session_id}.json"))


def delete_session_file(session_id: str):
    path = os.path.join(p_sessions_dir(), f"{session_id}.json")
