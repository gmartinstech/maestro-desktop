"""The reported symptom, pinned: an expired provedor-ia session must not read as a bad model.

The bug chain was: token dies (10h Keycloak access token, no refresh) -> sync_custom_providers
skips registering the provedor-ia node rather than replay a dead bearer -> `cp-provedor-ia/...`
...
from backend.apps.agents.manager.guard_provedor_ia_session import (
    guard_provedor_ia_session,
    routes_through_provedor_ia,
)
...
P_ULTRA = "custom/provedor-ia/maestro-ultra"
...
def p_settings(token: str) -> AppSettings:
    return apply_provedor_ia_defaults(AppSettings(provedor_ia_token=token))


# --------------------------------------------------------------------------- Which runs the guard claims.

def test_guard_claims_the_provedor_ia_lane_only(no_env_token):
    settings = p_settings(P_LIVE)
    assert routes_through_provedor_ia(P_ULTRA, settings)
    assert not routes_through_provedor_ia("custom/lmstudio/llama", settings)
    assert not routes_through_provedor_ia("sonnet", settings)


def test_a_live_token_is_never_blocked(no_env_token):
    guard_provedor_ia_session(P_ULTRA, p_settings(P_LIVE))
...
def test_an_opaque_credential_is_never_blocked(no_env_token):
    ...
    guard_provedor_ia_session(P_ULTRA, p_settings("pi-static-api-key-000000"))
...
def test_a_neighbouring_custom_provider_is_untouched(no_env_token):
    ...
    guard_provedor_ia_session("custom/lm-studio/llama", settings)


# --------------------------------------------------------------------------- The reported symptom.

def test_expired_token_on_maestro_ultra_raises_the_session_error(no_env_token):
    with pytest.raises(ProvedorIaSessionExpiredError) as excinfo:
        guard_provedor_ia_session(P_ULTRA, p_settings(P_EXPIRED))
...
def test_a_cleared_token_still_raises_even_with_a_stale_entry(no_env_token):
    """apply_provedor_ia_defaults never deletes, so a cleared token leaves the old entry behind."""
    settings = p_settings(P_EXPIRED)
    settings.provedor_ia_token = ""
    with pytest.raises(ProvedorIaSessionExpiredError) as excinfo:
        guard_provedor_ia_session(P_ULTRA, settings)
...
def test_the_turn_fails_before_the_cli_spawns(no_env_token):
    ...
    with pytest.raises(ProvedorIaSessionExpiredError):
        asyncio.run(configure_provider_env(
            options_kwargs, session, "cp-provedor-ia/maestro-ultra", "custom", settings, [],
        ))
    assert "env" not in options_kwargs


def test_the_error_classifies_as_auth_and_not_as_an_unknown_model():
    err = ProvedorIaSessionExpiredError("expired")
...
def test_the_user_gets_the_sign_in_card_not_a_model_error(monkeypatch):
    session, events = p_drive_error(monkeypatch, ProvedorIaSessionExpiredError("expired"))
    payload = next(d for e, d in events if e == "agent:auth_error")
    assert payload["reason"] == "provedor_ia_token_expired"
