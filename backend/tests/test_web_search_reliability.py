...
reaches an entitled Anthropic endpoint: a DIRECT anthropic api-route model
(base_url = api.anthropic.com, user's key).

A SUBSCRIPTION-route Claude model (opus-4-8, route=None) sends the haiku call
through 9Router to a pool that 401s, so a bare anthropic_api_key in settings is
NOT enough. Those sessions must keep the free, always-working DDG fallback
instead of a 401-ing hosted path.
"""
...
def test_direct_anthropic_api_route_is_reliable():
    ...
    assert ok(uses_direct_anthropic_api=True) is True


def test_subscription_route_claude_is_NOT_reliable():
    # The exact bug: opus-4-8 (subscription route) + key in settings. The haiku call still 401s via the shared pool, so this must stay unreliable -> DDG.
    assert ok(uses_direct_anthropic_api=False) is False
