...
def test_registers_always_on_and_delegation_servers():
    ...
    # always-on
    assert "maestro-mcp-meta" in mcp_servers
    assert "maestro-settings-meta" in mcp_servers
    assert "maestro-apps" in mcp_servers
    # delegation (not denied)
    assert "maestro-browser-agent" in mcp_servers
    assert "maestro-invoke-agent" in mcp_servers
    ...
    for name in ("maestro-mcp-meta", "maestro-settings-meta", "maestro-apps",
                 "maestro-browser-agent", "maestro-invoke-agent"):
...
def test_fully_denied_delegation_servers_are_not_registered():
    ...
    assert "maestro-browser-agent" not in mcp_servers   # all browser tools denied -> skip
    assert "maestro-invoke-agent" not in mcp_servers
    assert "maestro-mcp-meta" in mcp_servers             # always-on regardless
    assert "maestro-apps" in mcp_servers                 # always-on regardless
