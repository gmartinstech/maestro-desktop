// Recovers the affiliate hash stamped into the installer's FILENAME (MaestroStudio-<arch>-<hash>.dmg/exe/AppImage), so attribution survives a different-browser or late first launch that the welcome-page cookie flow loses. Scans Downloads/Desktop for a recent stamped installer; anything other than exactly one match falls back to the welcome flow.
...
const FILENAME_ATTRIBUTION_WINDOW_MS =
  Number(process.env.MAESTRO_AFFILIATE_FILENAME_WINDOW_MS) || 30 * 24 * 60 * 60 * 1000;
const INSTALLER_HASH_RE =
  /^MaestroStudio(?:-Setup)?-(?:arm64|x64)-([A-Za-z0-9_-]{16,32})(?: \([0-9]+\))?\.(dmg|exe|AppImage)$/i;
