...
// Detach the CDP debugger from every webview of a browser card BEFORE React
// unmounts them. Otherwise a late DevTools notification (a Target child-session
// message or Network.responseReceivedExtraInfo) lands on a session Chromium has
// already freed and SIGSEGVs the whole browser process. Bounded + fail-open.
...
// Clean-detach a browser card's CDP, THEN remove it. All three card-removal
// paths (the X button, the agent-finish timer, and keyboard delete) route
// through here so none of them tears the webview down with the debugger still
// attached. The detach is bounded, so removal is never blocked by a dead pipe.
