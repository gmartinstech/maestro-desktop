...
import { hasAnyAgentLaunched, hasModelConnected } from './skipPredicates';
...
export const step03: OnboardingStep = {
  ...
  skipIf: (s) => hasAnyAgentLaunched(s) || !hasModelConnected(s),
  ...
  ops: [
    ...
    { kind: 'popup', text: tr('onboarding.steps.launch_agent.popup.openChat') },
    { kind: 'click', target: S.newAgentButton, simulate: true },
    { kind: 'move_to', target: S.chatInput },
    { kind: 'popup', text: tr('onboarding.steps.launch_agent.popup.askQuestion') },
    {
      kind: 'wait_user',
