...
import { colorForWorkflow, useWC, statusChip, statusDot } from '../shared/uiKit';
import { clockOf, whenText } from '../shared/model';
import WorkflowTitle from '../ui/WorkflowTitle';
...
const HomeView: React.FC<{ nav: AppNav }> = ({ nav }) => {
  ...
  const ongoing = useMemo(() => active.map((a) => {
    ...
    return {
      wfId: a.workflow_id,
      title: wf?.title || a.title || t('workflows.homeView.workflowDefaultTitle'),
      color: wf ? colorForWorkflow(wf) : WC.accent,
      stepLabel: total > 0 ? t('workflows.homeView.stepLabel', { step: Math.min(idx + 1, total), total }) : t('workflows.homeView.running'),
      pct,
      nowText: run?.last_tool_label || 'Working…',
      clock: a.started_at ? clockOf(new Date(a.started_at)) : '',
    };
  }), [active, items, allRuns, t]);
  ...
  const comingGroups = useMemo<ComingGroup[]>(() => {
    ...
    for (const e of events) {
      ...
      arr.push({ wfId: wf.id, title: wf.title || t('workflows.detailView.untitledWorkflow'), time: clockOf(f), sortKey: f.getTime(), steps: wf.steps.length, color: colorForWorkflow(wf) });
      byDay.set(key, arr);
    }
    ...
    for (let i = 0; i < 7; i += 1) {
      ...
      if (runs.length) groups.push({ key, dayNum: d.getDate(), dow: d.toLocaleDateString([], { weekday: 'short' }), runs, countLabel: t('workflows.homeView.comingRunsCount', { count: runs.length }) });
    }
    return groups;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [events, items, t]);
  ...
  const recents = useMemo(() => allRuns.slice(0, 8).map((r) => ({
    id: r.id,
    wfId: r.workflow_id,
    title: items[r.workflow_id]?.title || t('workflows.homeView.workflowDefaultTitle'),
    status: r.status,
      ...
      <div style={{ flex: 1, overflowY: 'auto', minHeight: 0, padding: '0 30px 32px' }}>
        {ongoing.length > 0 && (
          <div style={{ marginTop: 22 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 11 }}>
              <span style={{ fontFamily: "'Newsreader',serif", fontSize: 18, fontWeight: 500, color: WC.ink }}>{t('workflows.homeView.ongoingRuns')}</span>
              ...
              {ongoing.map((o) => (
                <div key={o.wfId} onClick={() => dispatch(openWorkflowMonitor({ workflowId: o.wfId }))} style={{ display: 'flex', alignItems: 'center', gap: 13, background: WC.raised, border: `1px solid rgba(${WC.inkRGB},0.10)`, borderRadius: WC.radius.md, padding: '11px 15px', cursor: 'pointer' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5, color: WC.accent, fontSize: 12, fontWeight: 600, flex: 'none' }}>
                    <span>{t('workflows.homeView.watch')}</span>
        ...
        {missed.length > 0 && (
          <div style={{ minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 11 }}>
              <span style={{ fontFamily: "'Newsreader',serif", fontSize: 18, fontWeight: 500, color: WC.ink }}>{t('workflows.homeView.missed')}</span>
              ...
              <div style={{ flex: 1 }} />
              <button onClick={reRunAll} style={{ background: 'transparent', border: `1px solid rgba(${WC.inkRGB},0.14)`, borderRadius: 8, padding: '6px 12px', fontSize: 12, fontWeight: 600, color: WC.ink3, cursor: 'pointer' }}>{t('workflows.homeView.reRunAll')}</button>
            </div>
              ...
              {missedVisible.map((m) => (
                <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 12, background: WC.raised, border: '1px solid rgba(194,72,58,0.20)', borderRadius: WC.radius.md, padding: '10px 14px' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11.5, color: WC.muted, marginTop: 1 }}>{t('workflows.homeView.missedWhileAppClosed')}</div>
                  </div>
                  ...
                  <button onClick={() => dispatch(runMissedRuns([m.id]))} style={{ background: WC.ink, color: WC.paper, border: 'none', borderRadius: 8, padding: '6px 13px', fontSize: 12, fontWeight: 600, cursor: 'pointer', flex: 'none' }}>{t('workflows.homeView.reRun')}</button>
                  <div onClick={() => dispatch(dismissMissedRuns([m.id]))} title={t('common.dismiss')} style={{ width: 26, height: 26, borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: WC.faint, flex: 'none' }}>
            ...
            {missed.length > 3 && (
              <button onClick={() => setMissedExpanded((v) => !v)} style={{ marginTop: 10, background: 'transparent', border: 'none', color: WC.danger, fontSize: 12.5, fontWeight: 600, cursor: 'pointer', padding: '4px 2px', display: 'flex', alignItems: 'center', gap: 6 }}>
                <span>{missedExpanded ? t('workflows.homeView.showLess') : t('workflows.homeView.showAll', { count: missed.length })}</span>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontFamily: "'Newsreader',serif", fontSize: 18, fontWeight: 500, color: WC.ink, marginBottom: 12 }}>{t('workflows.homeView.comingUp')}</div>
          {comingGroups.length === 0 && (
            <div style={{ fontSize: 13, color: WC.muted }}>{t('workflows.homeView.nothingScheduled')}</div>
          )}
              ...
              <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
                {(expandedDays.has(g.key) ? g.runs : g.runs.slice(0, COMING_CAP)).map((r, i) => (
                  <div key={`${r.wfId}-${i}`} onClick={() => nav.selectWorkflow(r.wfId)} style={{ display: 'flex', alignItems: 'center', gap: 13, background: WC.raised, border: `1px solid rgba(${WC.inkRGB},0.08)`, borderRadius: WC.radius.md, padding: '12px 15px', cursor: 'pointer' }}>
                    ...
                    <span style={{ fontSize: 12, color: WC.muted }}>{r.steps} {t('workflows.homeView.steps')}</span>
                ...
                {g.runs.length > COMING_CAP && (
                  <button onClick={() => setExpandedDays((prev) => { const next = new Set(prev); if (next.has(g.key)) next.delete(g.key); else next.add(g.key); return next; })} style={{ alignSelf: 'flex-start', background: 'transparent', border: 'none', color: WC.muted, fontSize: 12.5, fontWeight: 600, cursor: 'pointer', padding: '3px 2px', display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span>{expandedDays.has(g.key) ? t('workflows.homeView.showLess') : t('workflows.homeView.moreRuns', { count: g.runs.length - COMING_CAP })}</span>
        ...
        <div style={{ marginTop: 18 }}>
          <div style={{ fontFamily: "'Newsreader',serif", fontSize: 18, fontWeight: 500, color: WC.ink, marginBottom: 12 }}>{t('workflows.homeView.recents')}</div>
          {recents.length === 0 && <div style={{ fontSize: 13, color: WC.muted }}>{t('workflows.homeView.noRuns')}</div>}
            ...
            {recents.map((r) => (
              <div key={r.id} onClick={() => nav.selectWorkflow(r.wfId)} style={{ display: 'flex', alignItems: 'center', gap: 13, padding: '11px 4px', borderBottom: `1px solid rgba(${WC.inkRGB},0.05)`, cursor: 'pointer' }}>
                ...
                <div style={{ width: 72, display: 'flex', justifyContent: 'flex-end', flex: 'none' }}>
                  <span style={statusChip(r.status, WC)}>{r.status === 'failure' ? t('workflows.homeView.failed') : r.status === 'success' ? t('workflows.homeView.success') : r.status}</span>
                </div>
