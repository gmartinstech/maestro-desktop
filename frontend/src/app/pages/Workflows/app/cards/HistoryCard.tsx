...
import { useWC, FONT_SERIF, statusChip, statusDot, statusLabel } from '../shared/uiKit';
import { toRunRow, whenText } from '../shared/model';
...
const HistoryCard: React.FC<{ workflowId: string; title: string }> = ({ workflowId, title }) => {
  ...
  return (
    <div style={{ background: WC.paper, border: `1px solid rgba(${WC.inkRGB},0.08)`, borderRadius: WC.radius.lg, padding: 16 }}>
      <div style={{ fontFamily: FONT_SERIF, fontSize: 16, fontWeight: 500, color: WC.ink, marginBottom: 12 }}>{t('workflows.history.title')}</div>
      {rows.length === 0 && <div style={{ fontSize: 12.5, color: WC.muted2 }}>{t('workflows.history.empty')}</div>}
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        {rows.map((r) => (
          <div key={r.id} onClick={() => dispatch(openWorkflowMonitor({ workflowId, runId: r.id }))} title={t('workflows.history.openRun')} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '9px 0', borderBottom: `1px solid rgba(${WC.inkRGB},0.05)`, cursor: 'pointer' }}>
